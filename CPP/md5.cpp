...
void byteReverse(unsigned char *buf, unsigned longs) {
    Unsigned32 t;
    do {
        t = (Unsigned32) ((unsigned) buf[3] << 8 | buf[2]) << 16 |
...
void MD5Update(MD5Context *ctx, unsigned char const *buf, unsigned len) {
    ...
    t = ctx->bits[0];
    if ((ctx->bits[0] = t + ((Unsigned32) len << 3)) < t)
    ...
    if (t) {
        ...
        MD5Transform(ctx->buf, (Unsigned32 *) ctx->in);
        buf += t;
        len -= t;
    }
...
void MD5Final(unsigned char digest[16], MD5Context *ctx) {
    ...
    *p++ = 0x80;
    ...
    ((Unsigned32 *) ctx->in)[14] = ctx->bits[0];
    ((Unsigned32 *) ctx->in)[15] = ctx->bits[1];
 ...
 */
void MD5Transform(Unsigned32 buf[4], Unsigned32 const in[16]) {
    register Unsigned32 a, b, c, d;
...
void md5_cs( const char *s, unsigned si, unsigned char *digest ) {
    MD5Context ctx;

    MD5Init( &ctx );
    MD5Update( &ctx, (const unsigned char *)s, si );
    
    MD5Final( digest, &ctx);
}

inline char alpha( unsigned char v ) {
    return v<10 ? '0'+v : 'A'+v-10;
}

std::string md5_digest_to_string( unsigned char *digest ) {
    //
    std::string res; res.resize( 16 * 2 );
    for(unsigned i=0;i<16;++i) {
        res[ 2*i   ] = alpha( digest[i]/16 );
        res[ 2*i+1 ] = alpha( digest[i]%16 );
    }
    return res;
}
