...
void write_primitive_code( ostream &ftc, const map<string,vector<PrimitiveFunction *> > &name_of_primitive_functions, map<string,PrimitiveClass *> &sar_name_to_class ) {
    // exec functions
            ...
            for(unsigned ai=0;ai<iter3->second.size();++ai) {
                ...
                for(unsigned num_arg=0;num_arg<pf->args.size();++num_arg) {
                    ...
                    if ( pf->args[num_arg].type=="any" )
                        ...
                        ftc << "    " << sar_name_to_class[pf->args[num_arg].type]->cpp_name << " &" << an << " = *reinterpret_cast<" << sar_name_to_class[pf->args[num_arg].type]->cpp_name << " *>( args[" << num_arg << "]->data );\n";
                }
                // self 
                if ( class_name.size() ) {
                    std::string cpp_name = sar_name_to_class[class_name]->cpp_name;
                    if ( class_name=="Bool" )
                ...
                if ( pf->ret.size() ) {
                    if ( pf->ret != "manual" ) {
                        ftc << "    char *ret_data;\n";
                        ftc << "    if ( return_var ) {\n";
                        ftc << "        const unsigned size_ret = GetSizeInBytesForLonelyVariableOf<" << sar_name_to_class[pf->ret]->cpp_name << ">::res;\n";
                        ...
                        ftc << "    }\n";
                        ftc << "    else {\n";
                        ftc << "        char tmp[ GetSizeInBytesForLonelyVariableOf<" << sar_name_to_class[pf->ret]->cpp_name << ">::res ];\n";
                        ftc << "        ret_data = tmp;\n";
                        ftc << "    }\n";
                        // cpp return variable
                        if ( pf->ret=="Bool" )
                            ...
                            ftc << "    " << sar_name_to_class[pf->ret]->cpp_name << " &ret = *reinterpret_cast<" << sar_name_to_class[pf->ret]->cpp_name << " *>( ret_data );\n";
                    }
                }
                else {
                    ftc << "    if ( return_var ) {\n";
...
void write_register_primitive_classes( ostream &ftc, const vector<PrimitiveClass> &primitive_classes, const vector<PrimitiveFunction> &primitive_functions, map<string,PrimitiveClass *> &sar_name_to_class ) {
    unsigned cpt_prim = 0;
    for(vector<PrimitiveClass>::const_iterator iter=primitive_classes.begin();iter!=primitive_classes.end();++iter,++cpt_prim) {
        ...
        for(unsigned i=0;i<primitive_functions.size();++i)
            nb_methods += ( primitive_functions[i].method_of == iter->sar_name );
        ftc << "    // ----------------------------- CLASS -----------------------------\n";
        // DefinitionData
        ...
        ftc << "    dd->name = Nstring(\"" << iter->sar_name << "\");\n";
        ftc << "    dd->pertinence = FLOAT64_MAX_VAL;\n";
        ftc << "    dd->min_nb_args = 0;\n";
        ftc << "    dd->nb_args = 0;\n";
        ftc << "    dd->next_with_same_name = NULL;\n\n";
        // type init
        ftc << "    global_data." << iter->sar_name << " = types->new_elem();\n";
        ftc << "    *dd->type_cache.new_elem() = global_data." << iter->sar_name << ";\n";
        ftc << "    global_data." << iter->sar_name << "->init( " << nb_methods + iter->attributes.size() << " );\n";
        ftc << "    global_data." << iter->sar_name << "->init_md5_attributes_control_sum( " << cpt_prim << " );\n";
        ftc << "    global_data." << iter->sar_name << "->def_data = dd;\n";
        ftc << "    global_data." << iter->sar_name << "->name = dd->name;\n";
        ftc << "    global_data." << iter->sar_name << "->data_size_in_bits = GetSizeInBitsOf<" << iter->cpp_name << ">::res;\n";
        ftc << "    global_data." << iter->sar_name << "->needed_alignement = NeededAlignementOf<" << iter->cpp_name << ">::res;\n";
        ftc << "    global_data." << iter->sar_name << "->nb_sub_objects = 0;\n";
        ftc << "    global_data." << iter->sar_name << "->reassign_using_mem_copy = " << iter->reassign_using_mem_copy << ";\n";
        ftc << "    global_data.primitive_classes[" << cpt_prim << "] = global_data." << iter->sar_name << ";\n\n";
...
void write_register_primitive_classes_attr( ostream &ftc, const vector<PrimitiveClass> &primitive_classes, const vector<PrimitiveFunction> &primitive_functions, map<string,PrimitiveClass *> &sar_name_to_class ) {
    // attributes
    for(vector<PrimitiveClass>::const_iterator iter=primitive_classes.begin();iter!=primitive_classes.end();++iter) {
        ftc << "    tmp_type = global_data." << iter->sar_name << ";\n";
        for(unsigned i=0;i<iter->attributes.size();++i) {
            ...
            else ftc << "    *reinterpret_cast<" << sar_name_to_class[iter->attributes[i].type]->cpp_name << " *>( t_def_v->v.data ) = " << iter->attributes[i].value << ";\n";
...
void write_direct_conv_to_bool( std::ostream &ftc, const vector<PrimitiveClass> &primitive_classes ) {
    ...
    for(unsigned i=0;i<primitive_classes.size();++i)
        if ( primitive_classes[i].sar_name!="Bool" and 
             primitive_classes[i].sar_name!="Def" and 
             primitive_classes[i].sar_name!="Property" and
             primitive_classes[i].sar_name!="Error" and
             primitive_classes[i].sar_name!="Void" and
             primitive_classes[i].sar_name!="Lambda" and
             primitive_classes[i].sar_name!="CompiledFunctionSet_" and
             primitive_classes[i].sar_name!="Op" and
             primitive_classes[i].sar_name!="Pthread" and
             primitive_classes[i].sar_name!="Semaphore" and
             primitive_classes[i].sar_name!="BlockOfTok" )
            ftc << "    if ( v->type == global_data." << primitive_classes[i].sar_name << " ) return bool( *reinterpret_cast<const " << primitive_classes[i].cpp_name << " *>(v->data) );\n";
    ftc << "    return -1;\n";
    ftc << "}\n";
}
...
void write_direct_conv_to_Float64( std::ostream &ftc, const vector<PrimitiveClass> &primitive_classes ) {
    ...
    for(unsigned i=0;i<primitive_classes.size();++i)
        if ( primitive_classes[i].sar_name!="Def" and
             primitive_classes[i].sar_name!="Property" and
             primitive_classes[i].sar_name!="Error" and
             primitive_classes[i].sar_name!="Void" and
             primitive_classes[i].sar_name!="Lambda" and
             primitive_classes[i].sar_name!="CompiledFunctionSet_" and
             primitive_classes[i].sar_name!="UntypedPtr" and
             primitive_classes[i].sar_name!="CFile" and
             primitive_classes[i].sar_name!="Op" and
             primitive_classes[i].sar_name!="Pthread" and
             primitive_classes[i].sar_name!="Semaphore" and
             primitive_classes[i].sar_name!="BlockOfTok" )
            ftc << "    if ( v->type == global_data." << primitive_classes[i].sar_name << " ) { res = Float64( *reinterpret_cast<const " << primitive_classes[i].cpp_name << " *>(v->data) ); return true; }\n";
    ftc << "    return false;\n";
    ftc << "}\n";
}
...
//             for(int n=0;n<3;++n) {
//                 // op
//                 ftc << "    global_data." << pc->sar_name << "->" << lst[n] << "_op = Op::new_variable_op( global_data." << pc->sar_name << " );\n";
//                 ftc << "    def_v = current_variable_sp++; def_v->init( global_data." << pc->sar_name << ", Variable::CONST ); def_v->transient_data->op = global_data." << pc->sar_name << "->" << lst[n] << "_op;\n";
//                 
//                 // variables
//                 ftc << "    global_data." << pc->sar_name << "->" << lst[n] << "_var = global_data." << pc->sar_name << "->" << lst[n] << "_op->variable_data();\n";
//                 ftc << "    global_data." << pc->sar_name << "->" << lst[n] << "_var->init( global_data." << pc->sar_name << ", Variable::CONST );\n";
//                 
//                 if ( pc->sar_name=="Rationnal" ) ftc << "    reinterpret_cast<" << pc->cpp_name << " *>(global_data." << pc->sar_name << "->" << lst[n] << "_var->data )->init(" << n << ");\n";
//                 else if ( pc->sar_name=="Bool" ) ftc << "    Bool( global_data." << pc->sar_name << "->" << lst[n] << "_var->data, 1 << 7 ) = " << n << ";\n";
//                 else ftc << "    *reinterpret_cast<" << pc->cpp_name << " *>(global_data." << pc->sar_name << "->" << lst[n] << "_var->data ) = " << n << ";\n";
//             }
//         }
//     }
// }

// void write_primitive_code_for_op( ostream &ftc, const PrimitiveFunction &pc, map<string,PrimitiveClass *> &sar_name_to_class ) {
//     if ( pc.ret.size() == 0 )
//         return;
...
// void write_primitive_code_for_op( ostream &ftc, const vector<PrimitiveFunction> &primitive_functions, map<string,PrimitiveClass *> &sar_name_to_class ) {
//     for(unsigned i=0;i<primitive_functions.size();++i) {
//         write_primitive_code_for_op( ftc, primitive_functions[i], sar_name_to_class );
//     }
// }
...
void write_generated_tok_methods_cpp( const char *file_name, const vector<TokDef> &tok_defs, vector<PrimitiveClass> &primitive_classes, vector<PrimitiveFunction> &primitive_functions ) {
    ...
    map<string,PrimitiveClass *> sar_name_to_class;
    for(unsigned i=0;i<primitive_classes.size();++i)
        sar_name_to_class[ primitive_classes[i].sar_name ] = &primitive_classes[i];
    
    ftc << "\n";
    
    // primitive exec and try code
    write_primitive_code( ftc, name_of_primitive_functions, sar_name_to_class );
    ...
    ftc << "    Type *tmp_type;\n\n";
    write_register_primitive_classes( ftc, primitive_classes, primitive_functions, sar_name_to_class );
    ...
    write_register_primitive_classes_attr( ftc, primitive_classes, primitive_functions, sar_name_to_class );
    ftc << "}\n";
